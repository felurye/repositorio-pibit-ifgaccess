...
void setup(){
  ...
  Serial.begin(38400); //INICIALIA A SERIAL DO ARDUINO
  esp8266.begin(38400); //INICIALIA A SERIAL DO ESP-01
  
  //sendData("AT+RST\r\n",2000,DEBUG); //REINICIA O ESP-01
   //sendData("AT+GRM\r\n",2000,DEBUG); //VERSÃO DO FIRWARE DO ESP
...
void loop(){
  ...
  digitalWrite(ledPin, state); //escreve no Led o estado do botão
  ...
  if(esp8266.available()){ //VERIFICA SE O ESP8266 ESTÁ ENVIANDO DADOS E SE ESTÁ DISPONÍVEL
    ...
    leituraAnalog1 = String(analogRead(1)); //VARIÁVEL RECEBE O VALOR LIDO NO PINO ANALÓGICO (VALOR É CONVERTIDO EM STRING ANTES DE SER ARMAZENADO)
     ...
     webpage+="<center><h1>Teste de Envio (ESP-01)</h1><h2> ESP8266</h2><h3>";
...
//Função que é chamada na interrupção
void blink() {
  state = !state;
}
